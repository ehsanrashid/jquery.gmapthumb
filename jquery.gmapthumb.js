...
(function ($)
{
    $.fn.gmapthumb = function (maps)
    {
        ...
        {
            var map = maps[index];
            if (!map || !(map instanceof google.maps.Map)) throw new Error("google.maps.Map object not found");
            ...
            mapContainer.delegate('div div div div div', 'mouseover', function (event)
            {
                ...
                if (imagesContainer.length === 0) return;

                imagesContainer.each(function ()
                {
                    var imagePosition = $(this).position();
                    if (imagePosition.left === currentElementPosition.left
                     && imagePosition.top === currentElementPosition.top)
                    {
                        matchedElement = this;
                        return false;
                    }
